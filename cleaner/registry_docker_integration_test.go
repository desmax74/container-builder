...
func dockerPullTagPushOnRegistryContainer(suite *DockerTestSuite) bool {
	...
	err := d.PullImage(TEST_IMAGE_SECOND)
	time.Sleep(2 * time.Second) // needed on CI
	if err != nil {
	...
	err = d.TagImage(TEST_IMAGE_SECOND_TAG, TEST_IMAGE_SECOND_LOCAL_TAG)
	if err != nil {
		assert.Fail(suite.T(), "Tag Image Failed", err)
		return false
	}

	err = d.PushImage(TEST_IMAGE_SECOND_LOCAL_TAG, REGISTRY_CONTAINER_URL_FROM_DOCKER_SOCKET, "", "")
	if err != nil {
